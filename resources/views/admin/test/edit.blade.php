...
<head>
  ...
  <link href="{{asset('css/output.css')}}" rel="stylesheet">
    <title>Edit Health Test - BeYourSelf</title>
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <div class="flex flex-col gap-10 px-5 mt-5">
                <div class="breadcrumb flex items-center gap-[30px]">
                    ...
                    <span class="text-[#7F8190] last:text-[#0A090B]">/</span>
                    <a class="text-[#7F8190] last:text-[#0A090B] last:font-semibold ">Edit Health Test</a>
                </div>
            </div>
            <div class="header flex flex-col gap-1 px-5 mt-5">
                <h1 class="font-extrabold text-[30px] leading-[45px]">Edit Health Test</h1>
                <p class="text-[#7F8190]">Provide high quality for mental health</p>
            </div>
            @if($errors->any())
                <ul>
            ...
            @endif
            <!-- product -->
            <form method="POST" enctype="multipart/form-data" action="{{route('dashboard.test.update', $test)}}" class="flex flex-col gap-[30px] w-[500px] mx-[70px] mt-10">
                @csrf
                @method('PUT')
                <div class="flex gap-5 items-center">
                    ...
                    <div class="relative w-[100px] h-[100px] rounded-full overflow-hidden ">
                        <div class="relative file-preview z-10 w-full h-full ">
                            <img src="{{Storage::url($test->cover)}}" class="thumbnail-icon w-full h-full object-cover" alt="thumbnail">
                        </div>
                        <span class="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 text-center font-semibold text-sm text-[#7F8190]">Icon <br>Test</span>
                    </div>
                ...
                <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="flex items-center w-[500px] h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        ...
                        <input value="{{$test->name}}" type="text" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none" placeholder="Write your better course name" name="name" required>
                    </div>
                </div>
                <div class="group/category flex flex-col gap-[10px]">
                    ...
                    <div class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        ...
                        <select id="category" class="pl-1 font-semibold focus:outline-none w-full text-[#0A090B] invalid:text-[#7F8190] invalid:font-normal appearance-none bg-[url('{{asset('images/icons/arrow-down.svg')}}')] bg-no-repeat bg-right" name="category_id" required>
                            <option value="{{$test->category->id}}" selected>{{$test->category->name}}</option>
                ...
                <div class="flex items-center gap-5">
                    <a href="{{route('dashboard.test.index')}}" class="w-full h-[52px] p-[14px_20px] bg-[#0A090B] rounded-full font-semibold text-white transition-all duration-300 text-center">Cancel</a>
                    <button type="submit" class="w-full h-[52px] p-[14px_20px] bg-[#5EC1F9] rounded-full font-bold text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Save Test</button>
                </div>
            </form>
        </div>
    </section>
