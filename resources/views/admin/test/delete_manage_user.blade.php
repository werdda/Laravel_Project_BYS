...
<head>
  ...
  <link href="{{asset('css/output.css')}}" rel="stylesheet">
    <title>Health Test Details - BeYourSelf</title>
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <div class="flex flex-col gap-10 px-5 mt-5">
                <div class="breadcrumb flex items-center gap-[30px]">
                    ...
                    <span class="text-[#7F8190] last:text-[#0A090B]">/</span>
                    <a class="text-[#7F8190] last:text-[#0A090B] last:font-semibold ">Health Test Details</a>
                </div>
            </div>
            <div class="header ml-[70px] pr-[70px] w-[940px] flex items-center justify-between mt-10">
                ...
                <div class="relative">
                    ...
                    <div class="dropdown-menu absolute hidden right-0 top-[66px] w-[270px] flex flex-col gap-4 p-5 border border-[#EEEEEE] bg-white rounded-[18px] transition-all duration-300 shadow-[0_10px_16px_0_#0A090B0D]">
                        ...
                        <a href="{{route('dashboard.test.edit', $test->id) }}" class="flex gap-[10px] items-center">
                            <div class="w-5 h-5">
                                <img src="{{asset('images/icons/note-favorite-outline.svg')}}" alt="icon">
                            </div>
                            <span class="font-semibold text-sm">Edit Health Test Details</span>
                        </a>
                        
                        <form method="POST"action="{{route('dashboard.test.destroy', $test)}}" >
                                    @csrf
                                    @method('DELETE')
                                    <a class="flex gap-[10px] items-center text-[#FD445E]">
                                        <div class="w-5 h-5">
                                        ...
                                        <button class="font-semibold text-sm">Delete Health Test</button>
                                    </a>
                                </form>
                    </div>
                </div>
            ...
            <div id="course-test" class="mx-[70px] w-[870px] mt-[30px]">
                <h2 class="font-bold text-2xl">Mental Health Tests</h2>
                <div class="flex flex-col gap-[30px] mt-2">
                    <a href="{{route('dashboard.test.create.question', $test)}}" class="w-full h-[92px] flex items-center justify-center p-4 border-dashed border-2 border-[#0A090B] rounded-[20px]">
                        <div class="flex items-center gap-5">
                            <div>
                    ...
                    @forelse ($questions as $question) 
                        <div class="question-card w-full flex items-center justify-between p-4 border border-[#EEEEEE] rounded-[20px]">
                            ...
                            <div class="flex items-center gap-[14px]">
                                <a href="{{route('dashboard.test_questions.edit', $question)}}" class="bg-[#0A090B] p-[14px_30px] rounded-full text-white font-semibold">Edit</a>
                                <form method="POST" action="{{route('dashboard.test_questions.destroy', $question)}}">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <p>Mental health test belum memiliki sebuah soal.
                        </p>
                    @endforelse
