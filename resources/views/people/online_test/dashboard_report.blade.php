...
<head>
  ...
  <title>Result Health Test - BeYourSelf</title>
            ...
            <div class="flex flex-col px-5 mt-5">
                <div class="w-full flex justify-between items-center">
                    <div class="flex flex-col gap-1">
                        <p class="font-extrabold text-[30px] leading-[45px]">Result Test</p>
                        <p class="text-[#7F8190]">Please complete all of the mental health test to view your complete analysis</p>
                    </div>
                </div>
            </div>
            <div class="course-list-container flex flex-col px-5 mt-[30px] gap-[30px]">
                <div class="course-list-header flex flex-nowrap justify-between pb-4 pr-10 border-b border-[#EEEEEE]">
                    <div class="flex shrink-0 w-[300px]">
                        <p class="text-[#7F8190]">Health Test</p>
                    </div>
                ...
                <div class="list-items flex flex-nowrap justify-between pr-10">
                    ...
                    <div class="flex shrink-0 w-[170px] items-center justify-center">
                        <p class="p-[8px_16px] rounded-full bg-[#D5EFFE] font-bold text-sm text-[#086DFE]">
                            {{ $test->category->name }}
                        </p>
                    </div>
                     @endif
                    ...
                    @if(!$test->canStart)
                        <span class="w-full h-[41px] p-[10px_20px] bg-gray-500 rounded-full font-bold text-sm text-white text-center">Not Ready</span>
                    @elseif(Auth::user()->hasActiveSubscription())
                        <a href="{{ route('dashboard.onlineTest.rapport.test', $test) }}" class="w-full h-[41px] p-[10px_20px] bg-indigo-950 rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Result</a>
                    @else
                        <a href="{{route('price')}}" class="w-full h-[41px] p-[10px_20px] bg-indigo-950 rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center" >Get Pro</a>
                    @endif
                    </div>
                </div>
                ...
                @endif
                @empty
                <p>Belum ada test kesehatan mental yang diberikan</p>
                @endforelse
            </div>
        </div>
    </section>
